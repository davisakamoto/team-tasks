...
class ViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.task_pending = Task.objects.create(title="Tarefa Pendente", person=self.person1, category=self.category_dev, deadline=self.today + timedelta(days=5))       
        self.task_late = Task.objects.create(title="Tarefa Atrasada", person=self.person1, deadline=self.today - timedelta(days=2))
        self.task_done = Task.objects.create(title="Tarefa Concluída", person=self.person2, done=True)
    ...
    def test_create_person_post(self):
        ...
        response = self.client.post(self.create_person_url, {'name': 'Carlos Dias', 'role': 'Designer'})
    ...
    def test_edit_person_post(self):
        ...
        edit_url = reverse('manager:editPerson', args=[self.person1.id])
        response = self.client.post(edit_url, {'name': 'Ana Lima Editada', 'role': 'Frontend Sr.'})
        self.assertEqual(response.status_code, 302)
        # atualiza no banco
        self.person1.refresh_from_db()
